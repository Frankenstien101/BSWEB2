...
<body>
<h3>ALL SITE INVOICE SUMMARY</h3>
...
<script>
    ...
    function fetchSellers() {
        ...
        fetch(`/HomePage/datafetcher/reports/getdatareports.php?action=getsites&company=${encodeURIComponent(companyId)}`)
            .then(res => {
...
function loadItems2(page = 1) {
   ...
   // console.log("sellers:", sellers);
    ...
    fetch(`/HomePage/datafetcher/reports/getdatareports.php?action=allsiteinvoicesummary&company=${companyId}&siteid=${siteid}&page=${page}&limit=${rowsPerPage}&sellers=${encodeURIComponent(sellers.join(','))}&datefrom=${encodeURIComponent(datefrom)}&dateto=${encodeURIComponent(dateto)}`)
        .then(response => {
            if (!response.ok) {
                hideLoader();
...
function renderPagination() {
   ...
   function exportToCSV() {
    ...
    const url = `/HomePage/datafetcher/reports/getdatareports.php?action=allsiteinvoicesummarycsv&export=csv` +

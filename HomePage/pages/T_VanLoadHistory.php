...
<div class="filter-container">
    ...
    <!-- Date Filter Card -->
    <div class="card text-bg-light" style="max-width: 100%; font-size: 9px;">
...
<!-- Table Card -->
<div class="card text-bg-light mt-1" style="max-width: 100%; height: 55vh; margin-bottom: 0.5rem; font-size: 9px;">
...
<div class="card-header">
  ...
  <label class="radio-label mr-1">
    ...
    <span class="radio-custom">ALL</span>
  </label>
...
function loadItems2(page = 1) {
    ...
    fetch(`/HomePage/datafetcher/transactions/Van_Load_History_getdata.php?action=vanalloc&company=${companyId}&siteid=${siteid}&page=${page}&limit=${rowsPerPage}&sellers=${encodeURIComponent(sellers.join(','))}&datefrom=${encodeURIComponent(datefrom)}&dateto=${encodeURIComponent(dateto)}`)
        .then(response => {
            if (!response.ok) {
                hideLoader();
...
document.querySelectorAll('input[name="status"]').forEach(radio => {
    radio.addEventListener('change', function() {
        ...
        rows.forEach(row => {
            const statusCell = row.cells[6].textContent.trim(); // STATUS column
...
function renderTable(data, currentPage = 1) {
    ...
    data.forEach((item, index) => {
        ...
        tr.innerHTML = `
          ...
          <td>
              <a href="#" class="loading-link" data-id="${item.LOADING_ID}">
                ${item.LOADING_ID || ''}
              </a>
          </td>
...
function openLoadingIdModal(loadingId) {
    // Change modal title
    document.getElementById('currentLoadingId').textContent = loadingId;

 const tbody = document.querySelector('#loadingdetails tbody');

  tbody.innerHTML = ''; // Clear previous rows

  fetch(`/HomePage/datafetcher/transactions/Van_Load_History_getdata.php?action=loaddetails&loadingid=${loadingId}`)
    .then(response => {
    ...
    //  loadedPOs = data; // Save globally
      ...
      data.forEach((item, index) => {
        ...
        tr.innerHTML = `
          ...
          <td>${item.SW || ''}</td>
          <td>${item.IT || ''}</td>
          <td>${item.PRICE ? parseFloat(item.PRICE).toLocaleString('en-PH', { minimumFractionDigits: 0, maximumFractionDigits: 0 }) : '0'}</td>
          <td>${item.SIH_IT || ''}</td>
        <td>${item.TOTAL_CS_AMOUNT ? parseFloat(item.TOTAL_CS_AMOUNT).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}</td>
        <td>${item.TOTAL_IT_AMOUNT ? parseFloat(item.TOTAL_IT_AMOUNT).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00'}</td>
        `;
        tbody.appendChild(tr);
      });
    })

...
<body>
<h3>ALL SITE INVOICE DETAILED</h3>
...
function loadItems2(page = 1) {
    ...
    fetch(`/HomePage/datafetcher/reports/getdatareports.php?action=allsiteinvoicedetailed&company=${companyId}&siteid=${siteid}&page=${page}&limit=${rowsPerPage}&sellers=${encodeURIComponent(sellers.join(','))}&datefrom=${encodeURIComponent(datefrom)}&dateto=${encodeURIComponent(dateto)}`)
        .then(response => {
            if (!response.ok) {
                hideLoader();

...
function loaditems() {
    ...
    fetch(`/HomePage/datafetcher/reports/getdatareports.php?action=purchasereturnreport&company=${encodeURIComponent(companyId)}&siteid=${encodeURIComponent(siteid)}&datefrom=${encodeURIComponent(dateFrom)}&dateto=${encodeURIComponent(dateTo)}`)
        .then(response => {
        ...
        .then(data => {
            ...
            if (!data || data.length === 0) {
                ...
                tr.innerHTML = '<td colspan="17" class="text-center">No items found.</td>';
        ...
        if (item.STATUS === 'RECEIVED') {
            ...
            statusClass = 'bg-red';
               ...
               <td>${item.COMPANY_ID || ''}</td>
                ...
                <td>${item.GRN_TRANSACTION_ID || ''}</td>
                <td>${item.ITEM_ID || ''}</td>
                <td>${item.DESCRIPTION || ''}</td>
                <td>${item.DAMAGE_CS || ''}</td>
                <td>${item.DAMAGE_SW || ''}</td>
                <td>${item.DAMAGE_IT || ''}</td>
                <td>${item.EXCESS_CS || ''}</td>
                <td>${item.EXCESS_SW || ''}</td>
                <td>${item.EXCESS_IT || ''}</td>
                <td>${item.MISSING_CS || ''}</td>
                <td>${item.MISSING_SW || ''}</td>
                <td>${item.MISSING_IT || ''}</td>
...
function exportToExcel() {
    ...
    XLSX.writeFile(workbook, "IntransitReportSummary.xlsx");
}
